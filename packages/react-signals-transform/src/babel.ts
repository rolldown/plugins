...
export const BABEL_METADATA_KEY = dataNamespace
...
type HookUsage = typeof UNMANAGED | typeof MANAGED_COMPONENT | typeof MANAGED_HOOK
...
const get = (pass: PluginPass, name: any) => pass.get(`${dataNamespace}/${name}`)
const set = (pass: PluginPass, name: string, v: any) => pass.set(`${dataNamespace}/${name}`, v)

const markChanged = (pass: PluginPass) => set(pass, 'changed', true)
...
function shouldTransform(
  ...
  function isComponentFunction(path: NodePath<FunctionLike>, functionName: string | null): boolean {
    return (
...
function isValueMemberExpression(path: NodePath<BabelTypes.MemberExpression>): boolean {
  return (
    (path.node.property.type === 'Identifier' && path.node.property.name === 'value') ||
    (path.node.property.type === 'StringLiteral' && path.node.property.value === 'value')
  )
}
...
function isSignalCall(path: NodePath<BabelTypes.CallExpression>): boolean {
  ...
  if (callee.isIdentifier()) {
    ...
    return (
      name === 'signal' || name === 'computed' || name === 'useSignal' || name === 'useComputed'
    )
  }
...
function getVariableNameFromDeclarator(path: NodePath<BabelTypes.CallExpression>): string | null {
...
function prependUseSignals<T extends FunctionLike>(
  ...
  const body = t.blockStatement([
    t.expressionStatement(t.callExpression(get(state, getHookIdentifier)(), args)),
  ])
...
function createImportLazily(
  ...
  return () => {
    if (isModule(path)) {
      ...
      const matchesImportName = (s: BabelTypes.ImportDeclaration['specifiers'][0]) => {
...
function detectJSXAlternativeImports(path: NodePath<BabelTypes.Program>, state: PluginPass) {
...
export default function signalsTransform(
  ...
  return {
    ...
    post(file) {
      file.metadata[BABEL_METADATA_KEY] = {
        changed: get(this, 'changed') === true,
      }
    },
    visitor: {
      Program: {
