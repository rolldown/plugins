...
const signalCallNames = new Set(['signal', 'computed', 'useSignal', 'useComputed'])
...
function getParseOptions(id: string, code: string) {
  ...
  if (cleanId.endsWith('.tsx')) {
    lang = 'tsx'
  } else if (cleanId.endsWith('.ts') || cleanId.endsWith('.mts') || cleanId.endsWith('.cts')) {
...
function getAssignmentTargetName(node: ESTree.AssignmentExpression): string | null {
...
function getFunctionName(
  ...
  return (
    getFunctionNodeName(node) ?? getFunctionNameFromParent(parentMap.get(node), parentMap, filename)
  )
}
  ...
  switch (node.type) {
    ...
    case 'ParenthesizedExpression':
      return (
        ...
        isOptedIntoSignalTracking(parentMap.get(node), comments, code, parentMap)
      )
  ...
  switch (node.type) {
    ...
    case 'ParenthesizedExpression':
      return (
        ...
        isOptedOutOfSignalTracking(parentMap.get(node), comments, code, parentMap)
      )
...
function shouldTransform(
  ...
  if (options.mode == null || options.mode === 'auto') {
    return info.maybeUsesSignal && (isComponentFunction || isCustomHookName(info.name))
  }
...
function isRequireCall(node: ESTree.Node | null | undefined, source: string): boolean {
  return (
...
function isSignalCall(node: ESTree.CallExpression): boolean {
  return node.callee.type === 'Identifier' && signalCallNames.has(node.callee.name)
}
...
function injectSignalName(
  ...
  const nameLiteral = createSignalNameLiteral(variableName, filename, lineOf, node.start)
  const objectLiteral = `{\n  name: ${nameLiteral}\n}`
...
function createUniqueIdentifier(topLevelBindings: Set<string>, base: string): string {
...
function findExistingHookBinding(program: ESTree.Program, importSource: string): string | null {
  for (const statement of program.body) {
...
function addHookImport(
  ...
  if (isCommonJS) {
    s.prepend(`var ${hookIdentifier} = require(${JSON.stringify(importSource)}).useSignals\n`)
    return
  }
...
function transformFunction(
  ...
  if (hookUsage === UNMANAGED) {
    const hookCall = createUseSignalsCall(hookIdentifier, null, options, info.name)
  ...
  const hookCall = createUseSignalsCall(hookIdentifier, hookUsage, options, info.name)
  if (body.type === 'BlockStatement') {
  ...
  s.appendLeft(body.start, `{var ${effectIdentifier} = ${hookCall};\ntry {\nreturn `)
  s.appendLeft(body.end, `;\n} finally {\n${effectIdentifier}.f();\n}\n}`)
}
  ...
  return {
    ...
    transform: {
      ...
      handler: withMagicString(function (s, id) {
        ...
        const signalCallsToName: Array<{ node: ESTree.CallExpression; variableName: string }> = []
        ...
        walkNode(program, null, (node) => {
          if (node.type === 'CallExpression') {
            if (
              ...
              isJSXAlternativeCall(node, jsxAlternatives.identifiers, jsxAlternatives.objects)
            ) {
              const info = findParentComponentOrHook(node, parentMap, functionInfoMap)
              if (info != null) {
                info.containsJSX = true
              }
            }

            if (options.experimental?.debug && isSignalCall(node) && !hasNameInOptions(node)) {
              const variableName = getVariableNameFromDeclarator(node, parentMap)
          ...
          if (node.type === 'MemberExpression' && isValueMemberExpression(node)) {
            const info = findParentComponentOrHook(node, parentMap, functionInfoMap)
            if (info != null) {
          ...
          if (node.type === 'ObjectPattern' && hasValuePropertyInPattern(node)) {
            const info = findParentComponentOrHook(node, parentMap, functionInfoMap)
            if (info != null) {
              info.maybeUsesSignal = true
            }
        ...
        const functionsToTransform = Array.from(functionInfoMap.values()).filter((info) =>
          shouldTransform(info, options, comments, s.original, parentMap),
        )

        if (functionsToTransform.length === 0 && signalCallsToName.length === 0) {
          return
        }
