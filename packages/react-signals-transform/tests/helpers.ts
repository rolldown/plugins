...
type ComponentNode = NodeTypes['FuncDeclComp' | 'FuncExpComp' | 'ArrowComp' | 'ObjectMethodComp']
...
const codeTitle = (...parts: Array<string | undefined>) => parts.filter(Boolean).join(' ')
...
function expressionComponents(config: ComponentCodeConfig): GeneratedCode[] {
  ...
  if ((config.properInlineName != null && !config.properInlineName) || !config.auto) {
    components.push({
...
export function objMethodComp(config: CodeConfig): GeneratedCode[] {
  ...
  if (config.auto) {
    return [
      ...
      {
        name: codeTitle(baseName, 'with computed literal name, jsx, and signal usage'),
        ...generateCode({
          type: 'ObjectMethodComp',
          name: "['App']",
          body: 'return <>{signal.value}</>',
...
export function objAssignComp(config: CodeConfig): GeneratedCode[] {
  ...
  if (!config.auto) {
    ...
    codeCases.push({
      name: codeTitle(baseName, 'function component with bad computed property name'),
      ...generateCode({
        type: 'MemberExpAssign',
        property: "['render']",
        body: generateCode({
    ...
    codeCases.push({
      name: codeTitle(baseName, 'function component with dynamic computed property name'),
      ...generateCode({
        type: 'MemberExpAssign',
        property: "['Comp' + '1']",
        body: generateCode({
    ...
    codeCases.push({
      name: codeTitle(baseName, 'function component with computed property name'),
      ...generateCode({
        type: 'MemberExpAssign',
        property: "['Comp']",
        body: generateCode({
