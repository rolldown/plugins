  ...
  for (const testCase of testCases) {
    ...
    it(`(${testId}) ${testCase.name}`, async () => {
      ...
      if (config.expectTransformed) {
        expected += 'import { useSignals as _useSignals } from "@preact/signals-react/runtime";\n'
...
function runGeneratedComponentTestCases(config: TestCaseConfig): void {
  ...
  config = {
    ...config,
    filename: config.useValidAutoMode ? '/path/to/Component.js' : 'C:\\path\\to\\lowercase.js',
  }
...
describe('React Signals Babel Transform', () => {
  ...
  describe('all mode transformations', () => {
    ...
    it('transforms require syntax', async () => {
      ...
      const expectedOutput = `
			...
			`
      await runTest(inputCode, expectedOutput, { mode: 'all' }, undefined, true)
    })
  ...
  describe('signal naming', () => {
    ...
    const runDebugTest = async (inputCode: string, expectedOutput: string, fileName: string) => {
      // @ts-expect-error
