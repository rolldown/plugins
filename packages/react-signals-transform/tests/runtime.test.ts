...
async function createComponent(
  ...
  if (/\busing\s+/.test(sourceCode)) {
    const transformed = await rolldownTransform(filename, sourceCode, {
      jsx: 'preserve',
      lang: filename.endsWith('.tsx') ? 'tsx' : filename.endsWith('.ts') ? 'ts' : 'jsx',
      sourceType: 'module',
      target: 'es2022',
    })
...
describe('react signals transform runtime', () => {
  ...
  it('should transform components created by Array.map that use signals', async () => {
    ...
    const name = signal('John')
    await render(React.createElement(App as any, { name }))
    expect(scratch.innerHTML).toBe('<div><div>Hello John</div><div>Goodbye John</div></div>')

    await act(async () => {
      name.value = 'Jane'
    })
    expect(scratch.innerHTML).toBe('<div><div>Hello Jane</div><div>Goodbye Jane</div></div>')
  })
})
