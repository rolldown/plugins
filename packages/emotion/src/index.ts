                      ...
                      apply: (getTarget) => {
                        ...
                        if (existingArgs.length <= 1) {
                          ...
                          if (secondArg.type === 'ObjectExpression') {
                            ...
                            const hasTrailingComma = objText.trimEnd().endsWith(',')
                            innerCallText = `${styledName}(${firstArgText}, { ${objText}${maybeComma(!hasTrailingComma)} target: "${target}", label: "${escapeJSString(createLabel(labelContext, false))}" })`
                          } else {
                            // Wrap with spread
